...
const Login = () => {
  const [state, setState] = useState("User"); // Role selection
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await login({ email, password, role: state.toUpperCase() });
    ...
    <div className="flex min-h-screen bg-white font-sans">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 bg-gray-50/50">
        <div className="w-full max-w-[420px] space-y-8">
            ...
            {/* Role Selector */}
             <div className="p-1 bg-gray-100/80 rounded-xl grid grid-cols-3 gap-1 mb-6">
                {LOGIN_ROLES.map((role) => (
                  <button
                    key={role.id}
                    onClick={() => setState(role.id)}
                    className={`
                      relative py-2 px-3 rounded-lg text-sm font-medium transition-all duration-200 ease-in-out flex items-center justify-center gap-2
                      ${state === role.id
